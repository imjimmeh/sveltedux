...
import type { AsyncState, AsyncThunkAction } from "./types.js";
...
export function createAsyncReducer<TData, TError = string, TArg = unknown>(
  asyncThunk: AsyncThunkAction<TArg, unknown>,
  options: {
    pending?: (
...
export function createAsyncSlice<
  ...
  TAsyncStates extends Record<string, AsyncState<unknown, unknown>>,
  ...
  >,
  TAsyncThunks extends Record<string, AsyncThunkAction<unknown, TState>>
>(config: {
  name: TName;
  ...
  const reducer = (
    ...
    for (const [thunkName, asyncThunk] of Object.entries(asyncThunks)) {
      ...
      if (action.type === thunk.fulfilled && isFulfilledAction(action)) {
        return {
          ...state,
          [thunkName]: {
            ...state[thunkName as keyof TAsyncStates],
