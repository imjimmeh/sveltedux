...
import { isFulfilled, isRejected } from "../thunks.js";
...
interface ThunkInfo<TState = any> {
  typePrefix: string;
  arg: unknown;
  thunk: AsyncThunkAction<unknown, TState>;
  retryKey: string;
}
...
export function createRetryMiddleware<TState = any>(
  thunkRegistry: Map<string, AsyncThunkAction<any, TState>>,
  options: {
    maxRetries?: number;
    retryDelay?: number;
  ...
  const handleRejectedAction = (
    ...
    const error =
      (action as ActionWithErrorPayload).error ||
      (action as ActionWithErrorPayload).payload;
