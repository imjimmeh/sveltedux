...
export function createAsyncMiddleware<TState = unknown>(
  ...
  const handlePendingAction = (action: ActionWithMeta, getState: () => TState): void => {
  ...
  const handleCompletedAction = (action: ActionWithMeta & ActionWithError, getState: () => TState): void => {
    const { requestId, typePrefix } = extractActionMetadata(action);
